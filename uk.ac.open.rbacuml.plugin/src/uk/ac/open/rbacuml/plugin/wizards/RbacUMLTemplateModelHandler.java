package uk.ac.open.rbacuml.plugin.wizards;
 ...
 * @generated
 */
public class RbacUMLTemplateModelHandler
    ...
    protected boolean preFileCreation(IProgressMonitor monitor, TemplateConfiguration config) {
        config.addContentCreator("uk.ac.open.rbacuml.plugin.contentCreator", new IContentCreator() { //$NON-NLS-1$
            ...
            public IStatus createContent(IProgressMonitor progressMonitor,
                ...
                if (resources.length > 0) {
                    ...
                    if (model != null) {
                        ...
                        Profile profile = RbacUMLUtil.getProfile();
                        if (profile == null) {
                            return new Status(Status.ERROR, "uk.ac.open.rbacuml.plugin", 
                            	"uk.ac.open.rbacuml.plugin: Profile " + //$NON-NLS-1$
                            	RbacUMLUtil.PROFILE_NAME + 
    ...
    protected boolean postFileCreation(IProgressMonitor progressMonitor, IFile[] files) {
        ...
        if (!enabledIds.contains(RbacUMLUtil.RBACUML_CORE_ACTIVITY_ID)) {
            enabledIds = new HashSet<String>(enabledIds);
            enabledIds.add(RbacUMLUtil.RBACUML_CORE_ACTIVITY_ID);
