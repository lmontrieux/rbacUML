 ...
 * @generated
 */
public class DeactivateRolesCallOperationActionEditHelperAdvice extends
    ...
    protected String getLocalizedName(EObject eObject) {
        ...
        Stereotype stereotype = element.getAppliedStereotype(RbacUMLElementTypes._DEACTIVATEROLES__CALLOPERATIONACTION.getStereotypeName());
    ...
    protected ICommand getBeforeReorientReferenceRelationshipCommand(
    	...
    	if (request.getNewRelationshipEnd() != null && request.getOldRelationshipEnd() != null && request.getReferenceOwner() != null && sourceFeatureName != null && elementType != null) {
    		if (ReorientRequest.REORIENT_TARGET == request.getDirection()) {
    			 if (!RbacUMLElementTypes.matchesTarget(elementType, request.getNewRelationshipEnd()) || relationshipAlreadyExists(request, sourceFeatureName, elementType)) {
                     return UnexecutableCommand.INSTANCE;
                 } else {
                	 return getReorientCommand(request, sourceFeatureName, elementType);
                 }
    		} else {
    			if (!RbacUMLElementTypes.matchesSource(elementType, request.getNewRelationshipEnd()) || relationshipAlreadyExists(request, sourceFeatureName, elementType)) {
                    return UnexecutableCommand.INSTANCE;
                } else {
                	return getReorientCommand(request, sourceFeatureName, elementType);
                }
