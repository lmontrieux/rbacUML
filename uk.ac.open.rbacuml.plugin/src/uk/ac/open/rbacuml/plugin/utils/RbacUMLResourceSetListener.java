...
import uk.ac.open.rbacuml.plugin.providers.RbacUMLViewProvider;
 ...
 * @generated
 */
public class RbacUMLResourceSetListener
    extends ResourceSetListenerImpl {

	/**
	 * @generated
	 */
    public RbacUMLResourceSetListener() {
            	...
            	if (baseElement != null && stereotype != null && feature instanceof EReference) {
            		...
            		if (oldValue != null) {
            			...
            			for (Edge edge : edges) {
            				...
            				Class edgeClass = (Class)RbacUMLViewProvider.edgeMap.get(edgeType);
							...
							try {
								...
								if (feature.getName().equals(edgeFeature) && RbacUMLElementTypes.matchesSource(edgeElementType, baseElement) && RbacUMLElementTypes.matchesTarget(edgeElementType, oldTargetElement)) {
									EObjectUtil.destroy(edge);
								}
							} catch (Exception e) {
								uk.ac.open.rbacuml.plugin.Activator.getDefault().getLog().log(new Status(IStatus.ERROR, "uk.ac.open.rbacuml.plugin", -1, e.getLocalizedMessage(), e)); //$NON-NLS-1$
							}	
            			}
