package uk.ac.open.rbacuml.dsml.plugin.wizards;
 ...
 * @generated
 */
public class RbacDSMLTemplateModelHandler
    ...
    protected boolean preFileCreation(IProgressMonitor monitor, TemplateConfiguration config) {
        config.addContentCreator("uk.ac.open.rbacuml.dsml.plugin.contentCreator", new IContentCreator() { //$NON-NLS-1$
            ...
            public IStatus createContent(IProgressMonitor progressMonitor,
                ...
                if (resources.length > 0) {
                    ...
                    if (model != null) {
                        ...
                        Profile profile = RbacDSMLUtil.getProfile();
                        if (profile == null) {
                            return new Status(Status.ERROR, "uk.ac.open.rbacuml.dsml.plugin", 
                            	"uk.ac.open.rbacuml.dsml.plugin: Profile " + //$NON-NLS-1$
                            	RbacDSMLUtil.PROFILE_NAME + 
                            	" cannot be found; confirm location against pathmap."); //$NON-NLS-1$
                        }
    ...
    protected boolean postFileCreation(IProgressMonitor progressMonitor, IFile[] files) {
        ...
        if (!enabledIds.contains(RbacDSMLUtil.RBACDSML_CORE_ACTIVITY_ID)) {
            enabledIds = new HashSet<String>(enabledIds);
            enabledIds.add(RbacDSMLUtil.RBACDSML_CORE_ACTIVITY_ID);
